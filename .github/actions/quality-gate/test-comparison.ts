...
const mockScores = [
  ...
  { id: "s1", name: "accuracy", value: 0.85, source: "llm_judge" as const, experimentRunId: "run1" },
  { id: "s2", name: "accuracy", value: 0.87, source: "llm_judge" as const, experimentRunId: "run1" },
  { id: "s3", name: "coherence", value: 0.75, source: "llm_judge" as const, experimentRunId: "run1" },
  
  // Current experiment scores (experimentRunId maps to run2)
  { id: "s4", name: "accuracy", value: 0.90, source: "llm_judge" as const, experimentRunId: "run2" },
  { id: "s5", name: "accuracy", value: 0.88, source: "llm_judge" as const, experimentRunId: "run2" },
  { id: "s6", name: "coherence", value: 0.65, source: "llm_judge" as const, experimentRunId: "run2" },
];
...
const expectedCurrentAccuracy = (0.90 + 0.88) / 2; // 0.89
const expectedCoherence = 0.75;
const expectedCurrentCoherence = 0.65;

if (Math.abs(scoresByEvaluator.get("accuracy")!.baseline.reduce((a, b) => a + b) / 2 - expectedAccuracy) < 0.001) {
...
if (Math.abs(scoresByEvaluator.get("accuracy")!.current.reduce((a, b) => a + b) / 2 - expectedCurrentAccuracy) < 0.001) {
