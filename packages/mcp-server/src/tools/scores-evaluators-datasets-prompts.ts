...
export function registerWorkflowTools(server: McpServer, api: FoxhoundApiClient): void {
    ...
    async (params) => {
      try {
        if (params.confirm !== true) {
          const lines: string[] = [`# Score Preview`, "", `**This will create the following score:**`, "", `- Trace ID: ${params.trace_id}`];
        ...
        const lines: string[] = [`# Score Created`, "", `✅ Successfully created score **${score.id}**`, "", `- Trace: ${score.traceId}`];
        ...
        return { content: [{ type: "text", text: `Error creating score for trace "${params.trace_id}": ${msg}` }] };
      }
    },
  );
    ...
    async (params) => {
      try {
        ...
        if (!response.data.length) {
          return { content: [{ type: "text", text: `# Scores for ${params.trace_id}\n\nNo scores found for this trace.` }] };
        }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: "text", text: `Error fetching scores for trace "${params.trace_id}": ${msg}` }] };
      }
    },
  );
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (!response.data.length) return { content: [{ type: "text", text: "No evaluators configured." }] };
  ...
  server.tool(
    ...
    "Trigger async evaluator runs for one or more traces. Evaluator runs are async — use foxhound_get_evaluator_run to check status and results.",
    {
      ...
      trace_ids: z.array(z.string()).min(1).max(50).describe("Array of trace IDs to evaluate (1-50)"),
    },
    async (params) => {
      try {
        const response = await api.triggerEvaluatorRuns({ evaluatorId: params.evaluator_id, traceIds: params.trace_ids });
        const lines: string[] = [
        ...
        for (const run of response.runs) lines.push(`- **${run.id}** → trace: ${run.traceId} | status: ${run.status}`);
        return { content: [{ type: "text", text: lines.join("\n") }] };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: "text", text: `Error fetching evaluator run "${params.run_id}": ${msg}` }] };
      }
    },
  );
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (!response.data.length) return { content: [{ type: "text", text: "No datasets found." }] };
  ...
  server.tool(
    ...
    "Add a trace to a dataset by extracting its input from root span attributes. Preview mode shows what will be added; set confirm=true to execute.",
    {
      ...
      expected_output: z.record(z.unknown()).optional().describe("Optional expected output for this dataset item"),
      metadata: z.record(z.unknown()).optional().describe("Optional metadata to attach to the dataset item"),
      confirm: z.boolean().optional().describe("Set to true to execute; omit or set to false for preview"),
    },
    async (params) => {
      try {
        ...
        if (params.confirm !== true) {
          ...
          if (params.expected_output) {
            lines.push(`**Expected Output:**`, "```json", JSON.stringify(params.expected_output, null, 2), "```", "");
          }
          if (params.metadata) {
            lines.push(`**Metadata:**`, "```json", JSON.stringify(params.metadata, null, 2), "```", "");
          }
  ...
  server.tool(
    ...
    "Automatically add traces to a dataset based on score criteria. Filter traces where a specific score matches a threshold condition.",
    {
      ...
      operator: z.enum(["lt", "gt", "lte", "gte"]).describe("Score comparison operator: lt (<), gt (>), lte (<=), gte (>=)"),
      threshold: z.number().describe("The score threshold value to compare against"),
      since_days: z.number().int().min(1).optional().describe("Only consider traces from the last N days (optional)"),
      limit: z.number().int().min(1).max(1000).optional().describe("Maximum number of traces to add (optional, default: API default)"),
    },
    async (params) => {
      try {
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        if (!prompts.length) {
          return { content: [{ type: "text", text: "No prompts found. Create one with `foxhound_create_prompt`." }] };
        }
  ...
  server.tool(
    ...
    "Resolve a prompt by name and label (defaults to 'production'). Returns the prompt content, model, config, and version number. Use this to inspect what prompt version is currently active.",
    {
      ...
      label: z.string().optional().describe("The label to resolve (default: 'production'). Common labels: production, staging, canary"),
    },
    async (params) => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: "text", text: `Error resolving prompt "${params.name}": ${msg}` }] };
      }
    },
  );
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        if (!versions.length) return { content: [{ type: "text", text: `No versions found for prompt ${params.prompt_id}.` }] };
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: "text", text: `Error listing versions for "${params.prompt_id}": ${msg}` }] };
      }
    },
  );
  ...
  server.tool(
    ...
    { name: z.string().describe("Prompt name (alphanumeric, hyphens, underscores only — e.g. 'support-agent')") },
    async (params) => {
      try {
        const prompt = await api.createPrompt(params.name);
        return { content: [{ type: "text", text: `Prompt created: **${prompt.name}** (${prompt.id})\n\nNext: add a version with \`foxhound_create_prompt_version\`.` }] };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
  ...
  server.tool(
    ...
    async (params) => {
      try {
        const version = await api.createPromptVersion(params.prompt_id, { content: params.content, model: params.model });
        return { content: [{ type: "text", text: `Version **v${version.version}** created for prompt ${params.prompt_id} (${version.id})\n\nNext: label it with \`foxhound_set_prompt_label\` (e.g. label "staging" or "production").` }] };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
  ...
  server.tool(
    ...
    "Set a label (e.g. 'production', 'staging') on a specific prompt version. If the label already exists on another version of the same prompt, it is moved. This is a write operation.",
    {
      ...
      label: z.string().describe("Label name (alphanumeric, hyphens, underscores — e.g. 'production', 'staging')"),
    },
    async (params) => {
      try {
        const result = await api.setPromptLabel(params.prompt_id, { label: params.label, versionNumber: params.version_number });
    ...
    async (params) => {
      try {
        ...
        if (!errorSpans.length) {
          return { content: [{ type: "text", text: `# Fix Suggestions: ${trace.id}\n\nNo errors detected in this trace. No fixes needed.` }] };
        }

        type ErrorCategory = "timeout" | "auth" | "rate_limit" | "tool_error" | "llm_error" | "validation" | "unknown";
        interface CategorizedError { category: ErrorCategory; span: Span; message: string; }
        ...
        for (const span of errorSpans) {
          ...
          const errorMsg = errorEvents.length > 0
            ? String(errorEvents[0]?.attributes["error.message"] ?? errorEvents[0]?.attributes["message"] ?? "").toLowerCase()
            : "";
          let category: ErrorCategory = "unknown";
          const duration = span.endTimeMs && span.startTimeMs ? span.endTimeMs - span.startTimeMs : 0;
          if (duration > 30000 || errorMsg.includes("timeout") || errorMsg.includes("timed out") || errorMsg.includes("deadline")) category = "timeout";
          else if (errorMsg.includes("unauthorized") || errorMsg.includes("forbidden") || errorMsg.includes("401") || errorMsg.includes("403") || errorMsg.includes("api key") || errorMsg.includes("authentication")) category = "auth";
          else if (errorMsg.includes("rate limit") || errorMsg.includes("429") || errorMsg.includes("too many requests") || errorMsg.includes("quota")) category = "rate_limit";
          else if (span.kind === "tool_call") category = "tool_error";
          else if (span.kind === "llm_call" || errorMsg.includes("model") || errorMsg.includes("openai") || errorMsg.includes("anthropic")) category = "llm_error";
          else if (errorMsg.includes("validation") || errorMsg.includes("invalid") || errorMsg.includes("required") || errorMsg.includes("schema")) category = "validation";
        ...
        if (byCategory.has("timeout")) {
          ...
          lines.push("", "**Suggested Fixes:**", "- Increase timeout threshold in your client configuration", "- Optimize the underlying query or operation to reduce latency", "- Add retry logic with exponential backoff", "- Consider breaking the operation into smaller chunks", "");
        }
        if (byCategory.has("auth")) {
          ...
          lines.push("", "**Suggested Fixes:**", "- Verify API key is set correctly in environment variables", "- Check that credentials haven't expired or been revoked", "- Ensure you have the necessary permissions for this operation", "- Confirm the API endpoint is correct for your key", "");
        }
        if (byCategory.has("rate_limit")) {
          const errors = byCategory.get("rate_limit")!;
          lines.push(`## Rate Limit Exceeded (${errors.length})`, "");
          for (const e of errors) lines.push(`**${e.span.name}**`);
          lines.push("", "**Suggested Fixes:**", "- Implement exponential backoff with jitter", "- Reduce request rate or batch operations", "- Upgrade your API plan for higher rate limits", "- Add request queuing to smooth traffic", "");
        }
        if (byCategory.has("tool_error")) {
          const errors = byCategory.get("tool_error")!;
          lines.push(`## Tool Execution Errors (${errors.length})`, "");
          for (const e of errors) lines.push(`**${e.span.name}**`);
          lines.push("", "**Suggested Fixes:**", "- Verify tool configuration and parameters are correct", "- Check that the tool service is available and responsive", "- Inspect tool-specific logs for detailed error messages", "- Validate tool input schema and required fields", "");
        }
        if (byCategory.has("llm_error")) {
          const errors = byCategory.get("llm_error")!;
          lines.push(`## LLM Call Failures (${errors.length})`, "");
          for (const e of errors) lines.push(`**${e.span.name}**`);
          lines.push("", "**Suggested Fixes:**", "- Check prompt length against model context window limits", "- Verify model name/identifier is correct and available", "- Confirm API quota hasn't been exceeded", "- Review model-specific error codes in LLM provider docs", "");
        }
        if (byCategory.has("validation")) {
          const errors = byCategory.get("validation")!;
          lines.push(`## Validation Errors (${errors.length})`, "");
          for (const e of errors) lines.push(`**${e.span.name}**`);
          lines.push("", "**Suggested Fixes:**", "- Review input schema and ensure all required fields are provided", "- Verify data types match the expected schema", "- Check for null/undefined values in required fields", "- Inspect detailed error messages for specific validation failures", "");
        }
        if (byCategory.has("unknown")) {
          ...
          lines.push("", "**Suggested Fixes:**", "- Inspect error events and attributes for more context", "- Check application logs for additional error details", "- Use `foxhound_explain_failure` for detailed error chain analysis", "");
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
