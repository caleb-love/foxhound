...
import { z } from "zod";
import { FoxhoundApiClient, toEpochMs } from "@foxhound/api-client";
function getConfig() {
...
function formatTraceList(result) {
    ...
    const lines = result.data.map((t) => {
        ...
        const duration = t.endTimeMs && t.startTimeMs ? `${((t.endTimeMs - t.startTimeMs) / 1000).toFixed(1)}s` : "?";
...
async function main() {
    ...
    // --- Tool: search traces ---
    server.tool("foxhound_search_traces", "Search traces by agent name, time range, and pagination. Returns a summary list of matching traces.", {
    ...
    // --- Tool: get trace ---
    server.tool("foxhound_get_trace", "Get the full trace with its complete span tree. Use this to inspect what happened during an agent run.", {
    ...
    // --- Tool: replay span ---
    server.tool("foxhound_replay_span", "Reconstruct the full agent state at the moment a specific span began — including LLM context, tool inputs, and memory. Requires Pro plan.", {
    ...
    // --- Tool: diff runs ---
    server.tool("foxhound_diff_runs", "Compare two agent runs side-by-side and surface divergence points. Useful for debugging regressions. Requires Pro plan.", {
    ...
    // --- Tool: get anomalies ---
    server.tool("foxhound_get_anomalies", "Surface behavioral anomalies in recent traces for an agent — unusually slow spans, error spikes, or unexpected tool usage patterns.", {
    ...
    // --- Tool: cost summary ---
    server.tool("foxhound_get_cost_summary", "Get token usage and cost breakdown. Shows current billing period span usage and limits.", {}, async () => {
    ...
    server.tool("foxhound_list_alert_rules", "List all alert rules configured for your organization.", {}, async () => {
        const data = await api.listAlertRules();
        const rules = data.data;
        if (!rules.length)
            return { content: [{ type: "text", text: "No alert rules configured." }] };
        const lines = rules.map((r) => `- **${r.id}** | ${r.eventType} >= ${r.minSeverity} -> channel ${r.channelId} | ${r.enabled ? "enabled" : "disabled"}`);
        return {
            content: [
    ...
    server.tool("foxhound_create_alert_rule", "Create a new alert rule that routes events to a notification channel. This is a write operation.", {
        event_type: z
    ...
    server.tool("foxhound_delete_alert_rule", "Delete an alert rule by ID. Set confirm=true to execute. Without confirmation, returns a preview of what will be deleted.", {
    ...
    // --- Tool: list channels ---
    server.tool("foxhound_list_channels", "List all notification channels (e.g. Slack webhooks) configured for your organization.", {}, async () => {
        const data = await api.listChannels();
        const channels = data.data;
        if (!channels.length)
            return { content: [{ type: "text", text: "No notification channels configured." }] };
        const lines = channels.map((c) => `- **${c.id}** | ${c.kind} | "${c.name}" | created ${c.createdAt}`);
        return {
            content: [
                {
                    type: "text",
    ...
    // --- Tool: create channel ---
    server.tool("foxhound_create_channel", "Create a new Slack notification channel. This is a write operation.", {
    ...
    // --- Tool: test channel ---
    server.tool("foxhound_test_channel", "Send a test alert through a notification channel to verify it works.", {
    ...
    // --- Tool: delete channel ---
    server.tool("foxhound_delete_channel", "Delete a notification channel by ID. This may also delete associated alert rules. Set confirm=true to execute.", {
    ...
    // --- Tool: list API keys ---
    server.tool("foxhound_list_api_keys", "List active API keys for your organization. Keys are masked — only the prefix is shown.", {}, async () => {
        const data = await api.listApiKeys();
        const keys = data.data;
        if (!keys.length)
            return { content: [{ type: "text", text: "No API keys found." }] };
        const lines = keys.map((k) => `- **${k.id}** | "${k.name}" | prefix: ${k.prefix}... | created ${k.createdAt}`);
        return {
            content: [{ type: "text", text: `## API Keys (${keys.length})\n\n${lines.join("\n")}` }],
        };
    });
    // --- Tool: create API key ---
    server.tool("foxhound_create_api_key", "Create a new API key. For security, the plaintext key is NOT returned through MCP — use the CLI (`foxhound keys create`) or dashboard to retrieve it.", {
    ...
    // --- Tool: revoke API key ---
    server.tool("foxhound_revoke_api_key", "Revoke an API key by ID. The key will immediately stop working. Set confirm=true to execute.", {
    ...
    server.tool("foxhound_get_agent_budget", "Get the cost budget configuration and current spend status for a specific agent.", {
    ...
    server.tool("foxhound_check_sla_status", "Check SLA targets and compliance status for a specific agent, including p95 duration and success rate.", {
    ...
    server.tool("foxhound_detect_regression", "Compare two versions of an agent and detect span-level regressions — missing or newly added spans between versions.", {
    ...
    // --- Tool: list baselines ---
    server.tool("foxhound_list_baselines", "List all stored baseline snapshots for an agent, showing version, sample size, and creation date.", {
        ...
        try {
            ...
            const lines = [
                ...
                "",
                ...data.data.map((b) => `- **v${b.agentVersion}** | sample size: ${b.sampleSize} | created: ${new Date(b.createdAt).toISOString()}`),
            ];
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (err) {
    ...
    // --- Tool: status ---
    server.tool("foxhound_status", "Check the Foxhound API health and show usage summary for the current billing period.", {}, async () => {
    ...
    server.tool("foxhound_explain_failure", "Analyze a failed trace and provide a detailed explanation of what went wrong, including the error chain, affected spans, and timing information.", {
        ...
        try {
            ...
            const firstError = errorSpans.reduce((earliest, current) => current.startTimeMs < earliest.startTimeMs ? current : earliest);
    ...
    // --- Tool: suggest fix ---
    server.tool("foxhound_suggest_fix", "Analyze a failed trace and suggest specific fixes based on error patterns, including timeout issues, auth failures, rate limits, and more.", {
        trace_id: z.string().describe("The trace ID to analyze"),
    }, async (params) => {
        try {
            const trace = await api.getTrace(params.trace_id);
            ...
            for (const span of errorSpans) {
                ...
                const errorMsg = errorEvents.length > 0
                    ? String(errorEvents[0]?.attributes["error.message"] ??
            ...
            const lines = [`# Fix Suggestions: ${trace.id}`, ""];
            // Timeout
            if (byCategory.has("timeout")) {
                ...
                for (const e of errors) {
                    const duration = e.span.endTimeMs && e.span.startTimeMs ? e.span.endTimeMs - e.span.startTimeMs : 0;
            ...
            if (byCategory.has("unknown")) {
                ...
                for (const e of errors) {
                    ...
                    const msg = errorEvents.length > 0
                        ? String(errorEvents[0]?.attributes["error.message"] ??
                            errorEvents[0]?.attributes["message"] ??
                            "Unknown error")
                        : "Unknown error";
    ...
    // --- Tool: score trace ---
    server.tool("foxhound_score_trace", "Create a score for a trace or specific span. Scores can be numeric values (0-1) or categorical labels. Preview mode shows what will be created; set confirm=true to execute.", {
    ...
    server.tool("foxhound_get_trace_scores", "Retrieve all scores attached to a trace, including both trace-level and span-level scores.", {
    ...
    server.tool("foxhound_list_evaluators", "List all configured LLM-as-a-Judge evaluators with their settings.", {}, async () => {
        try {
            ...
            for (const evaluator of response.data) {
                ...
                lines.push(`| ${evaluator.id} | ${evaluator.name} | ${evaluator.model} | ${evaluator.scoringType} | ${enabled} |`);
            }
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (err) {
    ...
    // --- Tool: run evaluator ---
    server.tool("foxhound_run_evaluator", "Trigger async evaluator runs for one or more traces. Evaluator runs are async — use foxhound_get_evaluator_run to check status and results.", {
    ...
    server.tool("foxhound_get_evaluator_run", "Check the status and results of an async evaluator run.", {
    ...
    function extractTraceInput(trace) {
    ...
    // --- Tool: list datasets ---
    server.tool("foxhound_list_datasets", "List all datasets with their IDs, names, descriptions, and item counts.", {}, async () => {
        try {
            ...
            const datasetsWithCounts = await Promise.all(response.data.map(async (dataset) => {
                try {
    ...
    server.tool("foxhound_add_trace_to_dataset", "Add a trace to a dataset by extracting its input from root span attributes. Preview mode shows what will be added; set confirm=true to execute.", {
    ...
    // --- Tool: curate dataset ---
    server.tool("foxhound_curate_dataset", "Automatically add traces to a dataset based on score criteria. Filter traces where a specific score matches a threshold condition.", {
    ...
    // --- Tool: list prompts ---
    server.tool("foxhound_list_prompts", "List all prompt templates in the registry. Shows prompt names, IDs, and timestamps. Requires Pro plan.", {}, async () => {
        try {
            ...
            const lines = [
                ...
                "",
                ...prompts.map((p) => `- **${p.name}** (${p.id}) | updated ${new Date(p.updatedAt).toISOString()}`),
            ];
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (err) {
    ...
    // --- Tool: get prompt ---
    server.tool("foxhound_get_prompt", "Resolve a prompt by name and label (defaults to 'production'). Returns the prompt content, model, config, and version number. Use this to inspect what prompt version is currently active.", {
    ...
    server.tool("foxhound_list_prompt_versions", "List all versions of a prompt, including their labels. Use the prompt ID (not name) from `foxhound_list_prompts`.", {
    ...
    // --- Tool: create prompt ---
    server.tool("foxhound_create_prompt", "Create a new prompt in the registry. The name must be alphanumeric with hyphens/underscores (no spaces). Requires Pro plan. This is a write operation.", {
        name: z
            .string()
    ...
    server.tool("foxhound_create_prompt_version", "Add a new version to an existing prompt. The version number auto-increments. Requires Pro plan. This is a write operation.", {
        prompt_id: z.string().describe("The prompt ID (e.g. 'pmt_...')"),
    ...
    server.tool("foxhound_set_prompt_label", "Set a label (e.g. 'production', 'staging') on a specific prompt version. If the label already exists on another version of the same prompt, it is moved. This is a write operation.", {
        prompt_id: z.string().describe("The prompt ID (e.g. 'pmt_...')"),
