...
import type { ExecDecisionInput, ExecTalkingPoint } from '@/components/overview/executive-summary-v2';
...
export function buildFleetActionItems(
  ...
  if (metrics.criticalRegressions > 0) {
    items.push({
      ...
      context: overrides?.regressionContext ?? 'Start with regressions and traces to confirm whether the newest failures reflect prompt drift, tool instability, or a release candidate that should be held back.',
      severity: 'critical',
  ...
  if (metrics.slaRisks > 0) {
    items.push({
      ...
      context: overrides?.slaContext ?? 'Latency and success-rate posture now warrant direct inspection before the next release or traffic increase.',
  ...
  if (metrics.budgetOverspendUsd > 0) {
    items.push({
      ...
      context: overrides?.budgetContext ?? 'Inspect the cost hotspots before approving further rollout or promotion, especially if the improvement case is not yet decisive.',
  ...
  if (items.length === 0) {
    items.push({
      ...
      context: overrides?.promotionContext ?? 'No fleet-level blockers are active. Use experiments and prompt history to validate whether the next promotion is worth shipping.',
      severity: 'healthy',
    ...
    items.push({
      ...
      context: overrides?.promotionContext ?? 'Once active regressions and SLA pressure are understood, compare the current candidate set before changing any prompt labels or release posture.',
      severity: 'healthy',
      agentIds: ['support-agent'],
      actions: overrides?.promotionActions ?? [
        { label: 'Experiments', href: hrefs.experiments },
...
export function buildExecutiveDecisions(
  ...
  decisions.push({
    title: overrides?.promotionDecision?.title ?? 'Should the current leading candidate move toward promotion?',
    status: overrides?.promotionDecision?.status ?? (metrics.criticalRegressions > 0 ? 'watch' : 'on-track'),
    evidence: overrides?.promotionDecision?.evidence ?? (metrics.criticalRegressions > 0
    ...
    recommendation: overrides?.promotionDecision?.recommendation ?? (metrics.criticalRegressions > 0 ? 'Review experiments before promotion' : 'Promote with monitoring'),
    href: overrides?.promotionDecision?.href ?? hrefs.experiments,
    cta: overrides?.promotionDecision?.cta ?? 'Review experiments',
  });

  decisions.push({
    title: overrides?.reliabilityDecision?.title ?? 'Is reliability stable enough for the next release window?',
    status: overrides?.reliabilityDecision?.status ?? (metrics.slaRisks > 0 ? 'attention' : 'on-track'),
    evidence: overrides?.reliabilityDecision?.evidence ?? (metrics.slaRisks > 0
    ...
    recommendation: overrides?.reliabilityDecision?.recommendation ?? (metrics.slaRisks > 0 ? 'Hold releases until reliability stabilizes' : 'No reliability hold required'),
  ...
  decisions.push({
    ...
    status: overrides?.budgetDecision?.status ?? (metrics.budgetOverspendUsd > 500 ? 'attention' : metrics.budgetOverspendUsd > 0 ? 'watch' : 'on-track'),
    evidence: overrides?.budgetDecision?.evidence ?? (metrics.budgetOverspendUsd > 0
    ...
    recommendation: overrides?.budgetDecision?.recommendation ?? (metrics.budgetOverspendUsd > 0 ? 'Review budgets before expanding traffic' : 'No additional budget action required'),
