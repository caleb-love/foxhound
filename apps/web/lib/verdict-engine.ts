...
function buildHeadline(severity: VerdictSeverity, regressions: number, slaRisks: number): string {
  if (severity === "critical") {
    const parts: string[] = [];
    if (regressions > 0)
      parts.push(`${regressions} critical regression${regressions > 1 ? "s" : ""}`);
...
function buildNarrative(m: FleetMetrics): string {
  ...
  if (m.previousHealthPercent !== undefined) {
    ...
    if (delta < 0) {
      parts.push(
        `Fleet is ${m.healthPercent}% healthy, down from ${m.previousHealthPercent}% previously.`,
      );
    } else if (delta > 0) {
      parts.push(
        `Fleet is ${m.healthPercent}% healthy, up from ${m.previousHealthPercent}% previously.`,
      );
    } else {
  ...
  if (m.criticalRegressions > 0) {
    parts.push(
      `${m.criticalRegressions} critical regression${m.criticalRegressions > 1 ? "s" : ""} detected.`,
    );
  }
...
function buildRagHeadline(rag: RagStatus, m: FleetMetrics): string {
  ...
  if (rag === "amber") {
    const issues: string[] = [];
    if (m.criticalRegressions > 0)
      issues.push(`${m.criticalRegressions} active risk${m.criticalRegressions > 1 ? "s" : ""}`);
