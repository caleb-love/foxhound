...
describe('segment-url', () => {
  ...
  it('adds or replaces segment in url', () => {
    ...
    expect(upsertSegmentInUrl('/traces?foo=bar', 'Planner agent')).toBe('/traces?foo=bar&segment=Planner+agent');
  });

  it('removes segment for all traffic', () => {
    expect(upsertSegmentInUrl('/traces?segment=Planner+agent&foo=bar', null)).toBe('/traces?foo=bar');
